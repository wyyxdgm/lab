      ...
      if (targettag == 'BODY') {
        ...
        $(window).scroll(function() {
          if (obj[0]) {
            ...
            if (target_element_bottom < top + 3 || target_element_position.top > window_bottom - 3) {
              // console.log(obj[0],obj[0].paused)
              poster = obj.attr('poster');
              // console.log(poster, obj.length)
              if (!obj[0].paused) {
                try {
                  if (obj.data('byclick')) {
                    $(targetobj).find(obj.data('byclick')).removeClass('fadeOut').addClass('fadeIn').css('pointer-events', 'auto');
                  }
                  obj[0].pause();
                } catch (e) {
                  console.error(e);
