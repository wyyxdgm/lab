...
module.exports.resolveAssets = function(htmlStr, lang, host) {
  ...
  assetsList.forEach(function(item) {
    ...
    let rep = host && (item.path.indexOf('img/') >= 0 || item.path.indexOf('fonts/') >= 0);
    let targetUrl = (rep ? host : item.pre) + '/' + path.relative(path.join(__dirname, '../src/'), _path) + '?' + getFileMd5(_path).substr(-5);
    rep && console.log(targetUrl);
  ...
  if (host) htmlStr = htmlStr.replace(/\.\.\/img/g, host + '/img');
  return htmlStr;
