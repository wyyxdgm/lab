  ...
  joinToGetMostMatchedRect([x, y], shape, rLikes, bLikes) {
    ...
    for (var i = rLikes.length - 1; i >= 0; i--) {
      ...
      for (var i = bLikes.length - 1; i >= 0; i--) {
        ...
        if (this.checkShapeAt(brShape, lx, ty, { similarValue: 38, positive_not_match_count: 17, nagtive_match_count: 17 })) {
          // 找到直接返回，看效果，不行优化。
  ...
  lineTo([x, y], shape, dir) {
    ...
    while (!(errors > shape.borderRadius + shape.borderWidth) && checkBounds(forX, forY, this.width, this.height)) {
      if (isEqualRGBA(shape.color, this.data[forX][forY])) {
        // this.drawRectToFile(new ShapeRect([x, y, searchedShape.w, searchedShape.h]), path.join(__dirname, `../test/output/{name}.png`));
        if (this.checkShapeAt(searchedShape, x, y, { similarValue: 8, positive_not_match_count: 17, nagtive_match_count: 17 })) {
          likes.push([x, y]);
        }
      } else {
        errors++;
    ...
    if (likes.length > LINE_MOST_MATCH_COUNT) {
      // 筛选
      console.log('likes.length > LINE_MOST_MATCH_COUNT && TODO filter the mosts')
    }
    return likes;
  }
