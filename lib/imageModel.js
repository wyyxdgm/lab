...
class ImageModel {
  ...
  groupByContinuous(points) {
    ...
    while ((keys = Object.keys(pointsMap)) && keys.length) {
      ...
      console.log('group ok:', group.length, arr.length);
      group.push(arr);
    }
  ...
  findOutlineToFile(file, x = 0, y = 0) {
    ...
    const rects = pointsGoupArray.map(points => ShapeRect.fromPointsBounds(points));
    const rectsPoints = rects.map(re => re.getBorderPoints());
    ...
    // }
    this.drawWidthPointsGroupToFile(file, [...pointsGoupArray, ...rectsPoints]);
  }
