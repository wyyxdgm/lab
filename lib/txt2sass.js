...
let buildCssCodeFromStyleObj = (styleObj, className = 'className') => {
    let str = `.${className} {\n`;
...
// pc sass func
let buildPCSassCodeFromStyleObj = (styleObj, className = 'className') => {
...
// H5 sass func
let buildH5SassCodeForFromStyleObj = (styleObj, className = 'className') => {
    let str = buildCssCodeFromStyleObj(styleObj, className);
    let str2 = `${str}`;
...
module.exports.buildTxtToScss = (srcPath, distPath, colorAliasMap) => {
    fs.readFile(srcPath, function(err, data) {
        ...
        groupedStr.forEach(function(data, index) {
            ...
            str += buildCssCodeFromStyleObj(styleObj, `className${index}`) + '\n';
            str2 += buildPCSassCodeFromStyleObj(styleObj, `className${index}`) + '\n';
            str3 += buildH5SassCodeForFromStyleObj(styleObj, `className${index}`) + '\n';
        });
        gutil.log('\n======================================================\n')
        gutil.log('\n' + [str, str2, str3].join('\n---------------------------\n'));
        gutil.log('\n======================================================\n')
