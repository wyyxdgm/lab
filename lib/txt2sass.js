...
module.exports.toH5 = (srcPath, distPath, colorAliasMap) => {
  fs.readFile(srcPath, function(err, scss) {
    let result = scss.toString();
    result = result.replace(/(fs)\(([\-0-9\+px]+)\)/g, '$1_($2)');
    result = result.replace(/(w)\(([\-0-9\+px]+)\)/g, '$1_($2)');
    result = result.replace(/(h)\(([\-0-9\+px]+)\)/g, '$1_($2)');
    result = result.replace(/(lh)\(([\-0-9\+px]+)\)/g, '$1_($2)');
    result = result.replace(/(ptb)\(([\-0-9\+px]+)\)/g, '$1_($2)');
    result = result.replace(/(plr)\(([\-0-9\+px]+)\)/g, '$1_($2)');
    result = result.replace(/(mtb)\(([\-0-9\+px]+)\)/g, '$1_($2)');
    result = result.replace(/(mlr)\(([\-0-9\+px]+)\)/g, '$1_($2)');
    result = result.replace(/(ml)\(([\-0-9\+px]+)\)/g, '$1_($2)');
    result = result.replace(/(mt)\(([\-0-9\+px]+)\)/g, '$1_($2)');
    result = result.replace(/(mr)\(([\-0-9\+px]+)\)/g, '$1_($2)');
    result = result.replace(/(mb)\(([\-0-9\+px]+)\)/g, '$1_($2)');
    result = result.replace(/(pl)\(([\-0-9\+px]+)\)/g, '$1_($2)');
    result = result.replace(/(pt)\(([\-0-9\+px]+)\)/g, '$1_($2)');
    result = result.replace(/(pr)\(([\-0-9\+px]+)\)/g, '$1_($2)');
    result = result.replace(/(pb)\(([\-0-9\+px]+)\)/g, '$1_($2)');
    result = result.replace(/(wh)\(([\-0-9\+px]+)\)/g, '$1_($2)');
    // wh(1px, 2px);
    // progress
    // swiper-dots-bottom
    // red-button('.button', 28px, 102px);
    // starsize(27px,25px,2px);
    // $content-width -> $content-width-h5
    result = result.replace(/(\s+)(\-?\w+px)/g, '$1rem($2)');
    result = result.replace(/rem\((\w+px)\)\-(\w+px)/g, 'rem($1 - $2)');
    fs.writeFile(distPath, result, () => {
      gutil.log(`${srcPath}: toH5 success!`);
    });
  });
}